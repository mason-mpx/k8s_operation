    ...
    stages {
        ...
        stage('Upload Artifact') {
            ...
            steps {
                ...
                script {
                    // 压缩源代码为 tar.gz
                    sh "tar -czf app-source.tar.gz --exclude='venv' --exclude='__pycache__' --exclude='.git' --exclude='*.pyc' ."
                    def uploadResp = sh(
                        script: """
                            curl -s -X POST '${params.ARTIFACT_UPLOAD_URL}' \\
                                -F 'file=@app-source.tar.gz' \\
        ...
        success {
            script {
                def msg = params.ENABLE_SONAR
                    ? "Python 项目构建成功 | SonarQube: ${env.SONAR_QUALITY_GATE_STATUS ?: 'SKIPPED'}"
                    : 'Python 项目构建成功'
