...
type CicdArtifact struct {
	...
	RunID      int64  `gorm:"column:run_id;index" json:"run_id"`           // 关联运行记录
	...
	ArtifactType string `gorm:"column:artifact_type;size:20" json:"artifact_type"` // 制品类型
	Version      string `gorm:"column:version;size:100" json:"version"`          // 版本号
	LanguageType string `gorm:"column:language_type;size:20" json:"language_type"` // 语言类型
	...
	// 状态
	Status        string `gorm:"column:status;size:20;default:'ready'" json:"status"`
